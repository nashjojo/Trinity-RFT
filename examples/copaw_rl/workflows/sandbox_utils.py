...
def _relax_e2b_key_validation_for_private_domain() -> None:
    """Accept private-sandbox API keys that lack the public ``e2b_`` prefix.

    The public E2B SDK requires keys shaped like ``e2b_<hex>`` (e2b.dev cloud).
    Self-hosted / private sandboxes (e.g. PAI-EAS) issue raw hex keys without
    that prefix. When a custom private ``E2B_DOMAIN`` is configured we relax the
    client-side format check so those keys are accepted. This is a no-op for the
    default e2b.dev path (``E2B_DOMAIN`` unset or ending in ``e2b.dev``), so
    public tutorial users keep the normal validation and error messages.
    """
    domain = os.environ.get("E2B_DOMAIN", "").strip()
    if not domain or domain.endswith("e2b.dev"):
        return
    try:
        import e2b.api as _e2b_api

        _e2b_api.validate_api_key = lambda *_args, **_kwargs: None
    except Exception:  # pragma: no cover - best-effort compatibility shim
        pass


_relax_e2b_key_validation_for_private_domain()
