...
@WORKFLOWS.register_module("queries_simple_workflow")
class QueriesSimpleWorkflow(MultiTurnWorkflow):
    ...
    def run(self) -> List[Experience]:
        ...
        is_over_length = (not is_transient) and traj_step_max_tokens > MAX_STEP_TOKENS

        if (is_over_length or is_over_time) and reward is not None:
            tags = []
            if is_over_length:
                tags.append("over_length")
            if is_over_time:
                tags.append("over_time")
            base_info["original_status"] = result.get("status")
            base_info["original_score"] = float(result.get("score") or 0.0)
            base_info["status"] = "+".join(tags)
            base_info["agent_call_seconds"] = agent_call_seconds
            base_info["max_step_tokens"] = traj_step_max_tokens
            reward = 0.0
            metrics["pass"] = 0.0
        metrics["over_length"] = 1.0 if is_over_length else 0.0
