# Agentic-RL 教程（配套可运行代码）

> 本仓库是《Agentic-RL：从黑盒到白盒》教程的**配套可运行代码**。框架本体 [Trinity-RFT](https://github.com/agentscope-ai/Trinity-RFT) 已作为依赖包引入（见 [`pyproject.toml`](./pyproject.toml)），本仓库只保留教程文档、实验配置、数据与一次跑通脚本。
...
| [`run.sh`](./run.sh) | 一次跑通脚本：`uv sync` 装环境 + 启动 CH1 复现实验 |
| [`examples/copaw_rl/`](./examples/copaw_rl/) | 实验代码：workflow 插件、config、数据、test_cases |
| [`scripts/tutorial/`](./scripts/tutorial/) | 第 2–5 章配套的拆解脚本（单次 rollout / reward / advantage / 权重更新） |

## 5 分钟跑通

前置：Python 3.11/3.12、[`uv`](https://github.com/astral-sh/uv)、可访问的 [E2B](https://e2b.dev/) sandbox，以及训练后端（Tinker 云服务或自部署 TuFT）。
...
- `pyproject.toml` 通过 `uv` 从 GitHub fork 固定 commit 安装 `trinity-rft[tinker]`。
  该 commit 含教程训练所需的两处修复（`tinker_trainer.py` logprobs 左截断、`tinker_model.py` tokenizer fallback），上游 PyPI 版本暂不包含。
- 密钥仅通过环境变量 / `.env` 注入，仓库内不含任何真实 key。

## 反馈

教程内容问题欢迎到 [Trinity-RFT](https://github.com/agentscope-ai/Trinity-RFT) 提 issue。
