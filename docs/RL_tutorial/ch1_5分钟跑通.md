...
| `DASHSCOPE_API_KEY` | 阿里云 DashScope 控制台 | sandbox 内 agent 调用的 LLM |
| `OSS_ACCESS_KEY_ID` | 阿里云 OSS 控制台 | 拉取压缩任务素材到 sandbox |
...
cd Trinity-RFT
cp .env.example .env
#   编辑 .env，填入上面拿到的 4 个 key（以及 Tinker 的 key 如果用云服务）
```
